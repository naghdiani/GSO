function result = GSO(func, dim, bound, max_iter, pop_size, r_a, p_c, p_m, info)
...
while (iter < max_iter)
    iter = iter + 1;
    grtrsnk_fitness = func(position);
    base_weight = mean(std(position));
    weight = squareform(pdist(position));
    
    if (min(grtrsnk_fitness) < g_best)
        [g_best, ind] = min(grtrsnk_fitness);
        g_best_pos = position(ind, :);
    end
    g_best_h = [g_best_h g_best];
    if (info && (iter == 1 || iter == 10 ...
        ...
        fprintf(['% 5s %.4e %.4e %.4e %.4e %02d:%02d:%02d.%03d\n'], num2str(iter), ...
            g_best, min(grtrsnk_fitness), base_weight, mean(mean(weight)), ...
            fix(elapsed_time / 3600000), mod(fix(elapsed_time / 60000), 60), ...
            mod(fix(elapsed_time / 1000), 60), (mod(elapsed_time, 1000)));
    end
    
    intensity_source = log(1 ./ (grtrsnk_fitness + 1E-100) + 1);
    intensity_attenuation = exp(-weight / (base_weight * r_a));
